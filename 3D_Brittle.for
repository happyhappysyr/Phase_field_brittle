...
c         STATEV(1) - H 
c
c     The energy decomposition is triggered via material name set in input.
c     Two options are available:   ELASTOPLASTIC_ISO or ELASTOPLASTIC_SD  
c     
...
c      
     ...
     2 0.5d0*EG*(stran(4)**2+ stran(5)**2 + stran(6)**2)
c	
c   ************************************************************************************************************   
c      
	  !SSE - Elastic energy
	  psiE_old = statev(1)                   !elastic history parameter value from the last converged increment
	  psiE_new = SSE !deformation energy value calculated in the 2nd layer for current iteration
c      
	  !SPD - Plastic energy
	  psiE_SPD = zero
c                  
